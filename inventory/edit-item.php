...
if (isset($_GET['id'])) {
    $item_id = $_GET['id'];
    try {
        $stmt = $db->prepare("SELECT * FROM inventory_items WHERE item_id = :item_id LIMIT 1");
        ...
        $message_type = 'error';
        error_log("Error fetching inventory item for edit: " . $e->getMessage());
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $item) {
    ...
    try {
        $stmt = $db->prepare("UPDATE inventory_items SET name = :name, description = :description, category = :category, unit = :unit, current_stock = :current_stock, minimum_stock = :minimum_stock, maximum_stock = :maximum_stock, unit_cost = :unit_cost, selling_price = :selling_price, supplier_id = :supplier_id, status = :status WHERE item_id = :item_id");
        ...
        if ($stmt->execute()) {
            ...
            // Re-fetch item data to show updated info
            $stmt = $db->prepare("SELECT * FROM inventory_items WHERE item_id = :item_id LIMIT 1");
            $stmt->bindParam(':item_id', $item_id);
            $stmt->execute();
            $item = $stmt->fetch(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Edit Inventory Item - Billing System</title>
        ...
        <main class="flex-1">
            ...
            <?php if ($item): ?>
                <div class="bg-white p-6 rounded-lg shadow-md">
                    <form action="edit-item.php?id=<?php echo htmlspecialchars($item['item_id']); ?>" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" rows="3"
                                      class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500"><?php echo htmlspecialchars($item['description'] ?? ''); ?></textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <input type="number" id="current_stock" name="current_stock" required
                                       value="<?php echo htmlspecialchars($item['current_stock']); ?>" min="0"
                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500">
                            </div>
                            <div>
                                ...
                                <input type="number" id="minimum_stock" name="minimum_stock"
                                       value="<?php echo htmlspecialchars($item['minimum_stock']); ?>" min="0"
                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="number" id="unit_cost" name="unit_cost" required step="0.01"
                                       value="<?php echo htmlspecialchars($item['unit_cost']); ?>" min="0"
                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500">
                            </div>
                            <div>
                                ...
                                <input type="number" id="selling_price" name="selling_price" required step="0.01"
                                       value="<?php echo htmlspecialchars($item['selling_price']); ?>" min="0"
                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500">
                            </div>
                        </div>
            ...
            <?php elseif (!$message): ?>
                <p class="text-gray-600 text-center">Loading item data...</p>
            <?php endif; ?>
        </main>
    </div>
</body>
